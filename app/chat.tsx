  ...
  const sendCommand = async (command: string, message: string) => {
    ...
    if (command === "run") {
      ws.send(
        JSON.stringify({
          command: "run",
          params: { workflow: "full", use_hardware: true },
        })
      );
  ...
  const sendMessage = async () => {
    if (input.trim()) {
      ...
      ws.send(
        JSON.stringify({
          command: "run",
          params: { workflow: "assistant", message: input },
        })
      );
  ...
  const renderMessage = (msg: string, index: number) => {
    ...
    return (
      ...
      >
        ...
        >
          <Markdown style={markdownStyles}>{cleanMessage}</Markdown>
        </View>
      </View>
    );
  };
